...
export const homeCapabilityGroups: HomeCapabilityGroup[] = [
  {
    ...
    items: [
      ...
      item(
        ...
        "Keepa-亚马逊-商品详情",
        "商品详情，一键获取",
        "亚马逊{{美国站}},查询 ASIN {{B0DD4GFNNG}} 的商品详情，输出标题、价格、评分、评论数、五点描述和 A+ 页面要点",
      ),
      item(
        keepaGroup,
        "keepa-price-history",
        "Keepa-亚马逊价格历史",
        "定价策略、价格历史记录",
        "亚马逊{{美国站}},查询 ASIN {{B0DD4GFNNG}} 的过去{{12个月}}价格历史记录，并按月份汇总输出",
      ),
    ],
  },
  {
...
function resolveCapabilityItems() {
  return dataSourceItems.length > 0 ? dataSourceItems : homeDataSourceItems;
}

function resolveCapabilityGroups() {
  return dataSourceGroups.length > 0 ? dataSourceGroups : homeCapabilityGroups;
}
...
export function getHomeCapabilityItem(id: string | null | undefined) {
  ...
  return resolveCapabilityItems().find((entry) => entry.id === normalized || entry.label === normalized) ?? null;
}

export function getHomeCapabilityCategory(id: string | null | undefined) {
  if (!id) return null;
  if (id === scenariosCategory.id) return scenariosCategory;
  return resolveCapabilityGroups().find((group) => group.id === id || group.label === id) ?? null;
}

export function getHomeCapabilityGroup(id: string | null | undefined) {
  if (!id) return null;
  const groupMatch = resolveCapabilityGroups().find((group) => group.id === id || group.label === id);
  if (groupMatch) return groupMatch;
  const itemMatch = getHomeCapabilityItem(id);
  if (!itemMatch) return null;
  return resolveCapabilityGroups().find((group) => group.id === itemMatch.parentId) ?? null;
}
...
export function getHomeCapabilityFilterIds(id: string | null | undefined) {
  ...
  const groupMatch = getHomeCapabilityGroup(id);
  return groupMatch ? groupMatch.items.map((entry) => entry.id) : [id];
}
