  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-[24px] font-semibold leading-8 text-[#111111]">角色权限</h1>
          <p className="mt-2 text-sm leading-6 text-[#747571]">管理后台角色，为角色分配不同的权限组合。</p>
        </div>
        <Button className="rounded-[10px]" onClick={openCreate}>新增角色</Button>
      </div>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-4 py-8 text-center text-[#94a3b8]">暂无角色</td></tr>
            ) : roles.map((role) => (
              <tr key={role.id} className="border-b border-[#f0f0ef] last:border-0">
                <td className="px-4 py-3 font-medium text-[#111111]">{role.name}</td>
                <td className="px-4 py-3 font-mono text-xs text-[#747571]">{role.code}</td>
                <td className="px-4 py-3 text-[#747571]">{role.description ?? "—"}</td>
                <td className="px-4 py-3 text-[#747571]">{role.user_count}</td>
                ...
                <td className="px-4 py-3 text-right">
                  <Button variant="ghost" size="sm" className="h-8 rounded-[8px]" onClick={() => openEdit(role)}>编辑</Button>
                  {!role.is_system && (
                    <Button variant="ghost" size="sm" className="h-8 rounded-[8px] text-red-600" onClick={() => setDeleteTarget(role)}>删除</Button>
                  )}
                </td>
              </tr>
            ))}
        ...
        <DialogContent className="max-w-lg rounded-[14px]">
          ...
          <div className="grid gap-3 pt-2">
            <div className="grid gap-1">
              <label className="text-xs text-[#747571]">角色名称</label>
            ...
            {!editTarget && (
              <div className="grid gap-1">
                ...
                <Input value={editCode} onChange={(e) => setEditCode(e.target.value)} className="h-9 rounded-[10px]" placeholder="如: super_admin" />
              </div>
            )}
            <div className="grid gap-1">
              <label className="text-xs text-[#747571]">描述</label>
              <Input value={editDesc} onChange={(e) => setEditDesc(e.target.value)} className="h-9 rounded-[10px]" />
            </div>
            <div className="grid gap-2">
              <label className="text-xs font-medium text-[#747571]">权限</label>
              <div className="max-h-[300px] overflow-auto rounded-[10px] border border-[#e2e2df] p-3">
                {Object.entries(groupedPerms).map(([resource, perms]) => (
                  <div key={resource} className="mb-3 last:mb-0">
                    <p className="mb-1 text-xs font-semibold text-[#52524f]">{resource}</p>
                    {perms.map((perm) => (
                      <label key={perm.id} className="flex items-center gap-2 py-0.5 text-sm text-[#111111]">
                        <Checkbox
                        ...
                        />
                        <span className="font-mono text-xs text-[#747571]">{perm.code}</span>
                        <span className="text-xs text-[#a1a1aa]">{perm.name}</span>
                      </label>
                    ))}
                  </div>
                ))}
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <DialogContent className="max-w-md rounded-[14px]">
          ...
          <p className="text-sm text-[#747571]">确定删除角色「{deleteTarget?.name}」？已分配此角色的用户将失去对应权限。</p>
