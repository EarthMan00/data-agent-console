...
/** 旧版任务执行卡片的交互容器，供 Durable Round 复用视觉和展开规则。 */
export function TaskExecutionPanel({
    ...
    <>
      ...
      {collapsible ? (
        <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-control text-foreground transition-colors group-hover:bg-fill-hover">
          <ChevronDown
            className={cn("h-5 w-5 transition-transform", expanded ? "rotate-180" : "rotate-0")}
            strokeWidth={1.5}
          />
        </span>
      ) : null}
  ...
  return (
    <div
      className="rounded-popover border border-border bg-bg-surface px-4 py-4 shadow-none"
      data-testid={testId}
    >
      {collapsible ? (
        <button
