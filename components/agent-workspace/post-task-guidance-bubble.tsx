...
export function PostTaskGuidanceBubble({
  ...
  /** 引导前的说明文案（如任务已终止提示），与引导 chip 同处一条 Alice 消息。 */
  leading?: string;
  content: string;
  datetime: string;
  ...
  if (suggestions.length === 0 && !leadingText) return null;
      ...
      <div className="w-full text-left">
        {leadingText ? (
          <div className="shrink-0 rounded-panel border border-border bg-bg-surface px-4 py-3 text-foreground shadow-none">
            <p className="text-body leading-7 text-foreground">{leadingText}</p>
          </div>
        ) : null}
        {suggestions.length > 0 ? (
          <div className={cn("space-y-2", leadingText && "mt-2")}>
            ...
            >
              {suggestions.map((item, index) => {
                ...
                return (
                  <div
                    key={`${index}-${item.slice(0, 24)}`}
                    className={cn(chipClass, "cursor-default border-border bg-bg-page text-text-tertiary opacity-80")}
                    aria-disabled="true"
                  >
                    <span className="whitespace-pre-wrap break-words">{item}</span>
                  </div>
